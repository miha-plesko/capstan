...
// * func(string)bool - file content is passed to this function and it must return true
...
//     c.Assert("/tmp/archive.tar.gz", TarGzEquals, map[string]string{"/file01.txt": func(val string)bool{return val == "Exact content"}})
...
//     c.Assert("/tmp/mydir", DirEquals, map[string]string{"/file01.txt": func(val string)bool{return val == "Exact content"}})
...
func compareMaps(obtained map[string]string, expected map[string]interface{}) error {
	...
	for key, val := range expected {
		...
		if expectedString, ok := val.(string); ok {
			...
			continue
		} else if expectedFn, ok := val.(func(string) bool); ok {
			if !expectedFn(obtained[key]) {
				return fmt.Errorf("mismatch for key '%s' (compare function returned false)", key)
			}
			continue
